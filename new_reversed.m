...
% theta = 30*pi/180;
% C = 9; D = 15;
% B = 0.5*(D-C +D*sec(theta)-C*sec(theta)*cos(alpha)); 
% A = 0.5*(D*(-1+(sec(theta)))+C*(1-sec(theta)*cos(alpha)));
%A = 0.5*(0.658*C-0.134*D); B = 0.5*(1.866*D-1.34*C); 
%A=2;B=8;
A = 15.6;B=A+17.9;C=10.15;D=sqrt(30^2+(17.9*sin(57*pi/180))^2);
...
% video = VideoWriter('Mechanism1.avi','Uncompressed AVI');
% open(video)
% writeVideo(video,F);
% close(video)
